...
from cross_models.attn import FullAttention, AttentionLayer, TwoStageAttentionLayer,TwoStageAttentionLayerV2
...
class scale_blockV2(nn.Module):
    ...
    def __init__(self, win_size, d_model, n_heads, d_ff, depth, dropout, \
                 seg_num=10, factor=10):
        super(scale_blockV2, self).__init__()
        ...
        self.encode_layers.append(TwoStageAttentionLayerV2(seg_num, factor, d_model, n_heads, \
                                                             d_ff, dropout))
    ...
    def forward(self, x):
        ...
        for layer in self.encode_layers:
            time_enc, dim_enc = layer(x)

        return time_enc, dim_enc
...
class Encoderv2(nn.Module):
    ...
    def __init__(self, e_blocks, win_size, d_model, n_heads, d_ff, block_depth, dropout,
        ...
        self.encode_blocks.append(scale_blockV2(1, d_model, n_heads, d_ff, block_depth, dropout, \
                                              in_seg_num, factor))
        # for i in range(1, e_blocks):
        #     self.encode_blocks.append(scale_blockV2(win_size, d_model, n_heads, d_ff, block_depth, dropout, \
        #                                           ceil(in_seg_num / win_size ** i), factor))
